...
def test_higuchi_estimator():
    ...
    (
        ...
        intercept,
        log_window_sizes,
        log_mean_normalized_lengths,
    ) = higuchi_estimator(X, min_window_size=10, max_window_size=100)
    ...
    for _ in range(100):
        X = np.random.randn(1000)
        hurst, _, _, _, _ = higuchi_estimator(
            X, min_window_size=10, max_window_size=100
        )
        hursts.append(hurst)
